...
public class RecognizerHttpClientWorker
{
    private static readonly Logger logger = LogManager.GetCurrentClassLogger();
    private bool isProjectRunning = false;
    private Project currentProject;
  
    private static RecognizerHttpClientWorker me = new RecognizerHttpClientWorker();
    public static RecognizerHttpClientWorker getInstance()
    {
        return me;
    }
    private void init()
    ...
    private RecognizerHttpClientWorker()
    {
        init();
    ...
    private void OnProjectStatusChange(object? sender, ProjectStatusEventArgs e)
    {
        if(e.State==ProjectState.start){
            ...
            foreach ((var module, var rdps) in remoteEndPoints)
            {
                ...
                var localModuleConfigCameraConfigs = ConfigUtil.getModuleConfig().CameraConfigs;
                        ...
                        var remoteModuleconfig = item.ModuleConfig.LowerConfig;
                            ...
                            foreach (var remoteLowerconfig in remoteModuleconfig)
                            {
                                if(!ConfigUtil.getModuleConfig().Standalone){
                                    foreach (var cameraConfig in localModuleConfigCameraConfigs)
                                    {
                                ...
                                {
                                    var recognizerConfig = ConfigUtil.getModuleConfig().RecognizerConfig.Columns;
                                   
                                        if (recognizerConfig[0] <= remoteLowerconfig.Columns[1] &&
                                            recognizerConfig[1] >= remoteLowerconfig.Columns[0])
                                        {
                                            found = true;
            ...
            if (upperRpcEndPoints.Count == 0)
            {
                if(ConfigUtil.getModuleConfig().Standalone){
                    throw new Exception($"Can not start project because no eligible upper is found! local recognizer coverage {string.Join(",",ConfigUtil.getModuleConfig().RecognizerConfig.Columns)}, Remote upper coverage {JsonConvert.SerializeObject(remoteEndPoints.Select(value=>value.Value.Select(value=>value.Value.ModuleConfig.LowerConfig.Select(value=>value.Columns))))}"); 
                }
                else
                {
                    throw new Exception($"Recognizer configured to work with camera, but camera config does not meet upper coverage Camera Config{JsonConvert.SerializeObject(ConfigUtil.getModuleConfig().CameraConfigs.Select(value=>value.Columns))} remote upper coverage  {JsonConvert.SerializeObject(remoteEndPoints.Select(value=>value.Value.Select(value=>value.Value.ModuleConfig.LowerConfig.Select(value=>value.Columns))))}");
                }

            }
            logger.Info("Recognizer ClientWorker initialized on project change event with eligible Upper {}, remote upper coverage {}",
                JsonConvert.SerializeObject(upperRpcEndPoints.Select(value=> $"{value.Address}:{value.WebPort} with lower column range {JsonConvert.SerializeObject(value.ModuleConfig.LowerConfig.Select(value=>value.Columns))}")), 
                JsonConvert.SerializeObject(remoteEndPoints
                    .Select(value=>value.Value
                        .Where(value=>value.Value.ModuleConfig.LowerConfig!=null)
                    .Select(value=>value.Value.ModuleConfig.LowerConfig
                        .Select(value=>value.Columns)))));
        }
    }
